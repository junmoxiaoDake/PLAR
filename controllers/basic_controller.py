...
class BasicMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        if self.args.agent in ['rnn',"mean_field"]:
    ...
    def init_hidden(self, batch_size):
        if self.args.agent in ['rnn', "mean_field"]:  # rnn-based agent
    ...
    def _build_inputs(self, batch, t):
        ...
        if hasattr(self.args,'choose_meanfield'):
            if self.args.choose_meanfield:
                mean_field_actions = batch["mean_field_actions"][:, t].unsqueeze(1)
                all_mean_field_actions = mean_field_actions.repeat(1 ,self.args.n_agents, 1)
                inputs.append(all_mean_field_actions)
    ...
    def _get_input_shape(self, scheme):
        ...
        if hasattr(self.args, 'choose_meanfield'):
            if self.args.choose_meanfield:
                input_shape += scheme["mean_field_actions"]["vshape"][0]
